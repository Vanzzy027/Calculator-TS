...
class Calculator {
    ...
    constructor() {
        ...
        this.display = document.getElementById('displayInput') as HTMLInputElement;
        this.buttonsContainer = document.getElementById('buttonsContainer') as HTMLElement;
        this.toggleTrack = document.querySelector('.toggle-track') as HTMLElement;
    ...
    private updateDisplay(value: string): void {
        const displayValue = value.replace(/\B(?=(\d{3})+(?!\d))/g, ','); // Add thousand separators
    ...
    private calculateResult(): void {
        ...
        try {
            ...
            let expression = this.currentInput.replace(/x/g, '*');
    ...
    private processInput(value: string): void {
        ...
        if (isOperator && (isLastCharOperator || this.currentInput === '')) {
            ...
            if (this.currentInput === '' && isOperator) return;
    ...
    private initTheme(): void {
        const savedTheme = localStorage.getItem('calcTheme') || 'theme-1';
    ...
    private setTheme(themeName: string): void {
        ...
        const themeNumber = themeName.split('-')[1]; // e.g., 'theme-1' -> '1'
    ...
    private handleThemeToggle(event: Event): void {
        ...
        let newThemeNumber: number;

        // Determine the target position
        if (target.classList.contains('toggle-btn')) {
            // Clicked the button itself, cycle to next theme
            const currentPos = parseInt(this.toggleTrack.getAttribute('data-position') || '1');
        ...
        this.setTheme(`theme-${newThemeNumber}`);
    }
}
